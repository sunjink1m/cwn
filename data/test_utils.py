...
def test_construction_of_ring_2complex(house_edge_index, include_coboundary_links):
    house = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    house.num_nodes = house_edge_index.max().item() + 1

    house_complex = compute_ring_2complex(house.x, house.edge_index, None, house.num_nodes,
                                          max_k=4, y=house.y, init_rings=True, include_coboundary_links=include_coboundary_links)
...
@pytest.mark.parametrize("include_coboundary_links", [True, False])
def test_construction_of_ring_2complex_with_edge_feats(house_edge_index, include_coboundary_links):
    ...
    house_complex = compute_ring_2complex(house.x, house.edge_index, house.edge_attr, house.num_nodes,
                                          max_k=4, y=house.y, init_rings=False, include_coboundary_links=include_coboundary_links)
...
@pytest.mark.parametrize("include_coboundary_links", [True, False])
def test_construction_of_ring_2complex_with_larger_k_size(house_edge_index, include_coboundary_links):
    ...
    house_cell_b = compute_ring_2complex(house.x, house.edge_index, None, house.num_nodes,
                                         max_k=10, y=house.y, init_rings=True, include_coboundary_links=include_coboundary_links)
...
@pytest.mark.parametrize("include_coboundary_links", [True, False])
def test_construction_of_ring_2complex_with_smaller_k_size(house_edge_index, include_coboundary_links):
    ...
    house_cell = compute_ring_2complex(house.x, house.edge_index, None, house.num_nodes,
                                       max_k=3, y=house.y, init_rings=True, include_coboundary_links=include_coboundary_links)
...
@pytest.mark.parametrize("include_coboundary_links", [True, False])
def test_ring_2complex_dataset_conversion(house_edge_index, include_coboundary_links):
    house1 = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    house2 = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    house3 = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    dataset = [house1, house2, house3]
    complexes, dim, num_features = convert_graph_dataset_with_rings(dataset, include_down_adj=True,
        init_rings=True, include_coboundary_links=include_coboundary_links)
    assert dim == 2
    assert len(num_features) == 3
    for i in range(len(num_features)):
        assert num_features[i] == 1
...
@pytest.mark.parametrize("include_coboundary_links", [True, False])
def test_ring_2complex_dataset_conversion_with_edge_feats(house_edge_index, include_coboundary_links):    
    edge_attr = torch.FloatTensor(
                        [[0.0, 1.0],
                         [0.0, 3.0],
                         [0.0, 1.0],
    ...
    complexes, dim, num_features = convert_graph_dataset_with_rings(dataset,
        init_edges=True, include_down_adj=True, init_rings=False, include_coboundary_links=include_coboundary_links)
    assert dim == 2
    assert len(num_features) == 3
...
@pytest.mark.parametrize("include_coboundary_links", [True, False])
def test_cell_complex_conversion_completes(include_coboundary_links):
    graphs = list(map(convert_to_graph, get_testing_complex_list()))
    _ = convert_graph_dataset_with_rings(graphs, init_rings=True, include_coboundary_links=include_coboundary_links)


def test_coboundary_links(house_edge_index):
    # Similar to test_construction_of_ring_2complex, except
    ...
    house_complex = compute_ring_2complex(house.x, house.edge_index, None, house.num_nodes,
                                          max_k=4, y=house.y, init_rings=True, include_coboundary_links=True)

    # size checks
    assert list(house_complex.cochains[0].coboundary_index.size()) == [2, 12]
    assert list(house_complex.cochains[1].coboundary_index.size()) == [2, 7]
    assert house_complex.cochains[2].coboundary_index is None
    ...
    assert t_params.kwargs['coboundary_index'] is None


    # Similar to test_ring_2complex_dataset_conversion, except
    # include_coboundary_links=True
    house1 = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    house2 = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    house3 = Data(edge_index=house_edge_index, x=torch.arange(0, 5, dtype=torch.float).view(5, 1), y=torch.tensor([1]))
    dataset = [house1, house2, house3]
    complexes, dim, num_features = convert_graph_dataset_with_rings(dataset, include_down_adj=True,
        init_rings=True, include_coboundary_links=True)

    for i in range(len(complexes)):
        # These checks are ones that I added on top of test_ring_2complex_dataset_conversion
        assert list(complexes[i].cochains[0].coboundary_index.size()) == [2, 12]
        assert list(complexes[i].cochains[1].coboundary_index.size()) == [2, 7]
        assert complexes[i].cochains[2].coboundary_index is None
