...
def test_cmp_aggregating():
    """
    Test whether the aggregation functions from Pytorch Geomteric are properly integrated 
    into CochainMessagePassing
    
    Eventually, the torch-geometric version of the repo can be updated to the latest one.
    In that version, the aggregator functions in MessagePassing is implemented differently as to
    also include other common aggregators such as softmax aggregator as well as mean, sum and max.
    We can use this test when incorporating that functionality into CochainMessagePassing.
    """
    ...
    expected_down_msg = torch.tensor([[14/3], [17/4], [11/3], [4/2], [11/3], [12/2], [23/3], [19/4], [15/2]], dtype=torch.float)
    assert torch.equal(down_msg, expected_down_msg)

    expected_up_msg = torch.tensor([[9/3], [34/7], [7/3], [6/3], [23/4], [22/4], [21/4], [20/4], [0]], dtype=torch.float)
    assert torch.equal(up_msg, expected_up_msg)

    expected_boundary_msg = torch.tensor([[3], [5], [7], [5], [8], [11], [13], [9], [15]], dtype=torch.float) / 2
    assert torch.equal(boundary_msg, expected_boundary_msg)

    expected_coboundary_msg = torch.tensor([[1], [3/2], [1], [1], [2], [2], [2], [2], [0]], dtype=torch.float)
    assert torch.equal(coboundary_msg, expected_coboundary_msg)
    ...
    expected_down_msg = torch.tensor([[8], [8], [5], [3], [6], [7], [9], [9], [8]], dtype=torch.float)
    assert torch.equal(down_msg, expected_down_msg)

    expected_up_msg = torch.tensor([[4], [8], [4], [3], [8], [8], [8], [7], [0]], dtype=torch.float)
    assert torch.equal(up_msg, expected_up_msg)

    expected_boundary_msg = torch.tensor([[2], [3], [4], [4], [5], [6], [7], [7], [8]], dtype=torch.float)
    assert torch.equal(boundary_msg, expected_boundary_msg)

    expected_coboundary_msg = torch.tensor([[1], [2], [1], [1], [2], [2], [2], [2], [0]], dtype=torch.float)
    assert torch.equal(coboundary_msg, expected_coboundary_msg)
